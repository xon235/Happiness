...
import com.family.happiness.viewmodels.toTokenData
import kotlinx.coroutines.launch
import timber.log.Timber
import java.lang.Exception

class SmsVerificationFragment : HappinessFragment() {
    ...
    private suspend fun signUp(tokenData: TokenData, profileData: ProfileData, smsCode: String) {
        try {
            val response = HappinessApi.retrofitService.signUp(SignUpData(tokenData, profileData, smsCode))
            if(response.result) {
